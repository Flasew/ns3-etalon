...
def getdatatransfered(root, dport):
    senderflows = set([f.attrib["flowId"] for f in root[1] if f.attrib["destinationPort"]==str(dport)])
    rsum = 0
    for f in root[0]:
        if f.attrib["flowId"] not in senderflows:
            rsum += int(f.attrib["rxBytes"])
    return rsum
...
with open(pdataname, "w") as pdata:
    for jname in [fname for fname in os.listdir(os.getcwd()) if fname[-4:]=="json"]:
        ...
        #pdata.write(str(fct) + "\t")
        rps = jdict["main_obj_success"]/20
        pdata.write(str(rps) + "\t")
        dps = getdatatransfered(xroot, 80)/20
        pdata.write(str(dps) + "\t")
        avg_lat = jdict["rx_delay_sum"]/jdict["main_obj_success"]
        pdata.write(str(avg_lat) + "\t")
        main_fail = jdict["main_obj_fail"]
        pdata.write(str(main_fail) + "\n")
        # retransmits = getretrans(xroot, 80)
        # pdata.write(str(retransmits) + "\t")
        # meanrtt = getmeanrtt(xroot, 80)
